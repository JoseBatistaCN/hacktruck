...
class PodcastTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    var podcasts = PodcastDAO.getLis()
    var filterPodcasts = [Podcast]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let podcastCell = cell as? PodcastTableViewCell {
            ...
            let podcast = filterPodcasts[indexPath.row]
            podcastCell.nomePodcastLabel.text = podcast.nomePodcast
            podcastCell.episodiosPoscastLabel.text = podcast.episodiosPodcast
    ...
    func searchBar(_ searchBar:UISearchBar, textDidChange searchText:String) {
        ...
        if searchText.isEmpty {
            ...
            filterPodcasts.removeAll()
        for podcast in podcasts {
            if podcast.nomePodcast.lowercased().contains(searchText.lowercased()) {
                filterPodcasts.append(podcast)
            }
        }
        }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            self.filterPodcasts.remove(at: indexPath.row)
